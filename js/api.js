    ...
    if (url.pathname === "/api/detail") {
      ...
      try {
        ...
        if (episodes.length === 0 && videoDetail.vod_content) {
          console.log(
            "标准json没有找到播放地址，尝试使用正则表达式查找m3u8链接"
          )
          const matches = videoDetail.vod_content.match(M3U8_PATTERN) || []
          episodes = matches.map((link) => {
            return {
              url: link.replace(/^\$/, ""),
              title: "",
            }
...
async function handleCustomApiSpecialDetail(id, customApi) {
  try {
    ...
    console.log("自定义api特殊详情页", matches)
    // 处理链接
...
async function handleSpecialSourceDetail(id, sourceCode) {
  try {
    ...
    let matches = []
    if (sourceCode === "jisu") {
      const vodLists = doc.querySelectorAll(".vod-list")
      if (vodLists[1]) {
        vodLists[1]
          .querySelectorAll(".list-title")
          .forEach((item) => matches.push(item.textContent.trim()))
      }
    } else {
      ...
      if (playLists[1]) {
        playLists[1]
          ...
          .forEach((item) => matches.push(item.value.trim()))
      }
    }
